                ...
                <div class="card m-b-30">
                    <div class="card-body">
                        <div class="addnewheader">
                            <a href="{{ route('addnew') }}" style="margin: 5px;">
                                <button type="button" class="btn btn-dark">Add Blog</button>
                            </a>

                            <button type="button" class="btn btn-success m-1">
                                Blogs <span class="badge bg-dark">{{ count( $blogs ) }}</span>
                            </button>
                        </div>
                    </div>
                </div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach ($blogs as $blog)
                                            <tr>
                                                <form action="{{ route('deleteBlog',$blog->id) }}" method="POST" enctype="multipart/form-data">
                                                    @csrf
                                                    <input type="text" name="id" value="{{ $blog->id }}" hidden>

                                                    <td><img src="{{ $blog->image }}" width="100px" style="border-radius: 1px;border: 1px solid #040b14;"></td>
                                                    <td>{{ $blog->title }}</td>
                                                    <td>{{ $blog->slug }}</td>
                                                    <td>
                                                        <a href="{{ route('edtiBlog',$blog->id) }}"><button type="button" class="btn btn-warning">Update</button></a>
